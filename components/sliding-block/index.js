// components/sliding-block/index.js
Component({
  /**
   * 组件的属性列表
   */
  ...
  methods: {
    ...
    slide_tap(e) {
      var that = this
      var imgIndex = Math.round(Math.random() * 2 + 1)
      that.setData({
      ...
      setTimeout(function () {
        ...
        // 滑块背景图
        context.drawImage(`../../images/verify/puzzle-bg-${imgIndex}.png`, 0, 0, context.width, context.height)
        context.draw(false, (() => {
    ...
    slide_chend(e) {
      ...
      if (((that.data.canfile_x + 5) > cliextX) && ((that.data.canfile_x - 5) < cliextX)) {
        ...
        that.triggerEvent('slidingBlock')
      } else {
        that.setData({
          slide_status: 3,
        })
